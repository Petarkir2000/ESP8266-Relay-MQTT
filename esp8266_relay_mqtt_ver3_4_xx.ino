...
// Function to generate a unique suffix from the MAC address
String getMacSuffix() {
  uint8_t mac[6];
  WiFi.macAddress(mac);
  char macSuffix[5];
...
// Defining constants with a unique suffix
#define CLIENT_ID ("ESP8266Relay_" + getMacSuffix()).c_str()
#define MQTT_TOPIC_CONTROL ("home/relay/control_" + getMacSuffix()).c_str()
#define MQTT_TOPIC_STATUS ("home/relay/status_" + getMacSuffix()).c_str()
#define MQTT_TOPIC_ONLINE ("home/relay/online_" + getMacSuffix()).c_str()
// - MQTT_WILL_TOPIC: Topic for publishing device status (ONLINE/OFFLINE).
#define MQTT_WILL_TOPIC ("home/relay/online_" + getMacSuffix()).c_str()
#define MQTT_WILL_DIAGNOSTICS ("home/relay/diagnostics_" + getMacSuffix()).c_str()
...
<p><strong>WiFi SSID:</strong> )rawliteral";
  ...
  html += mqtt_user;
  html += R"rawliteral(<br><strong>Device ID:</strong> )rawliteral";
  html += (getMacSuffix()).c_str(); 
...
// function to launch WiFi configuration portal
...
// Response to external commands given by command line
// They are used by the UPS to control the relay in the different phases of the UPS battery status and power status.
...
// Modified startWiFiConfigPortal function to accept parameters
  ...
  if (mqttConnected) {
    ...
    if (millis() - lastHeartbeat > HEARTBEAT_INTERVAL) {
      ...
      snprintf(payload, sizeof(payload), 
              ...
              WiFi.RSSI(), 
              ESP.getFreeHeap());
      client.publish(MQTT_WILL_DIAGNOSTICS, payload, true);
    }
  }
