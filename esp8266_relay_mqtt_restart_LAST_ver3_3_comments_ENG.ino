...
// On first boot (0xFF), default values are used.
  ...
  // Network selection and MQTT connection data input
  // WiFi configuration:
// - If no saved network, creates AP "Relay-Setup" for configuration.
// - User can set MQTT server, username and password.
// - On successful connection, settings are saved in EEPROM.
  WiFiManager wm;
...
// WiFi connection
  ...
  // WiFi auto-reconnect (should be after successful connection)
  WiFi.setAutoReconnect(true);    // Automatic reconnection on failure
  WiFi.persistent(true);          // Save WiFi settings in memory
  ...
  client.setSocketTimeout(5); // Timeout in seconds for MQTT operations
  ...
  // Manual WiFi start with saved settings (duplicate for reliability)
...
// - Checks WiFi connection and attempts reconnection if lost.
    ...
    if (mqttConnected) {
        ...
        if (millis() - lastHeartbeat > HEARTBEAT_INTERVAL) {
            ...
            // Publish heartbeat with correct parameters
            client.publish(MQTT_WILL_TOPIC, MQTT_WILL_ONLINE, MQTT_WILL_RETAIN);
            yield();
