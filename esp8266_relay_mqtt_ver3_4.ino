...
#define WIFI_RETRY_DELAY 3000      // 3 секунди забавяне между опитите
#define MAX_WIFI_RETRIES 5         // Максимален брой опити за повторно свързване
#define WIFI_CONNECTION_TIMEOUT 30000 // 30 секунди таймаут за връзка
...
// Добавете тази нова функция за стартиране на WiFi конфигурационен портал
void startWiFiConfigPortal() {
  ...
  // Ако сме успешно конфигурирали WiFi, запазваме настройките
  if (shouldSaveConfig) {
...
// - Checks WiFi connection and attempts reconnection if lost.
  ...
  // Подобрена проверка за WiFi връзка
  if (millis() - lastWiFiCheck > WIFI_CHECK_INTERVAL) {
    ...
    if (WiFi.status() != WL_CONNECTED) {
      ...
      // Проверяваме дали връзката е възстановена след 3 секунди
      delay(3000);
      if (WiFi.status() != WL_CONNECTED) {
